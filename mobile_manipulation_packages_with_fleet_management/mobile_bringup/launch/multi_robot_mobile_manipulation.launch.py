    ...
    for key, value in original_yaml.items():
        if key in global_keys:
            ...
            for prefix in prefixes:
                ...
                if 'root_name' in new_config and 'panda_' in new_config['root_name']:
                    clean_root = new_config['root_name'].replace("panda_", "")
                    new_config['root_name'] = f"{prefix}panda_{clean_root}"
...
def generate_controllers_dynamic(prefixes):
    ...
    for prefix in prefixes:
        ...
        ros2_ctrl["controller_manager"]["ros__parameters"][hand_controller] = {"type": "joint_trajectory_controller/JointTrajectoryController"}
        ...
        ros2_ctrl[hand_controller] = {"ros__parameters": {
            "joints": [
                f"{prefix}panda_finger_joint1", 
                f"{prefix}panda_finger_joint2"
            ],
            "command_interfaces": ["position"], "state_interfaces": ["position", "velocity"],
            "state_publish_rate": 100.0, "action_monitor_rate": 20.0
        }}
...
def generate_launch_description():
    ...
    a_star = Node(
        package="navigation", executable="a_star", name="a_star", 
    ...
    print("[LAUNCH] ✓ Base móvel habilitada para robot1")
    print("[LAUNCH] ✓ Fluxo de TF: world → robot1_base_link → robot1_panda_link0")
