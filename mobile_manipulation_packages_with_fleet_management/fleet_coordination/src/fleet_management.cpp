...
struct CollisionZone {
    ...
    float get_earliest_entry() const 
    {
        ...
        for(const auto& [rid, intervals] : robot_intervals) 
        {
            if(!intervals.empty()) 
            {
                if (intervals[0].first < min_t)
                {
                    min_t = intervals[0].first;
                }
            }
        }
        return min_t;
...
public:
    FleetManager() : Node("cbs_fleet_manager") 
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Fleet Manager Iniciado (Refatorado: Logica Separada da Visualizacao).");
...
private:
    ...
    // Tipo auxiliar para cache de assinatura
    ...
    // =============================================================================
    // CALLBACK PRINCIPAL (ORQUESTRADOR)
    // =============================================================================
    void fleet_callback(const mobile_manipulation_interfaces::msg::FleetPaths::SharedPtr msg)
    {
        // 1. Processamento de Lógica (Construção do Mapa e Resolução)
        auto start_time0_ = std::chrono::high_resolution_clock::now();
        ...
        {
            int r_id = 0;
            if (i < msg->robot_ids.size()) {
                r_id = msg->robot_ids[i];
            } else {
                r_id = (int)i;
            }
        ...
        // 2. Processamento de Visualização e Relatórios
        print_super_detailed_report(zones_vector, result.first, result.second);
        generate_visualization_trajectories(result.first);

        // Logs de performance
        std::chrono::duration<float> duration0 = end_time0 - start_time0_;
        std::chrono::duration<float> duration1 = end_time1 - start_time1_; 
        std::chrono::duration<float> duration2 = end_time2 - end_time1; // Correção aqui para medir apenas a resolução
        RCLCPP_INFO(this->get_logger(), "Time [Map Build]: %.6f s", duration0.count());
        RCLCPP_INFO(this->get_logger(), "Time [Detect]:    %.6f s", duration1.count());
        RCLCPP_INFO(this->get_logger(), "Time [Resolve]:   %.6f s", duration2.count());
    ...
    void request_new_scenario() 
    {
        ...
        scenario_client_->async_send_request(request, 
    ...
    std::vector<GridKey> get_footprint_keys(float x, float y) 
    {
        ...
        // Expande o raio (mesma lógica do process_robot_volume)
        while(toma <= config_.robot_radius) 
        {
            for(int eita = 0; eita <= opa * 2; eita++) 
            {   
                // Gera os 4 lados do losango/circulo discretizado
    ...
    void check_terminal_state_collisions() 
    {
        ...
        for(const auto& [robot_id, path] : original_paths_) 
        {
            ...
            // 1. Pega a coordenada final (Goal)
            std::pair<float, float> goal = path.back();

            // 2. Calcula quais células o robô ocupa quando está parado no Goal
            std::vector<GridKey> goal_footprint = get_footprint_keys(goal.first, goal.second);

            // 3. Verifica se alguma dessas células é usada por OUTRO robô
            ...
            for(const auto& key : goal_footprint) 
            {
                if(global_occupancy_map.count(key)) 
                {
                    ...
                    {
                        // Se a célula é visitada por ALGUÉM que não sou eu
            ...
            // 4. Reporta o problema
            if(!conflicting_robots.empty()) 
            {
                std::stringstream ss;
                ss << "PERIGO CRITICO: O destino do Robo " << robot_id 
                   << " em (" << goal.first << ", " << goal.second << ") "
                   << "esta dentro da zona de passagem/destino dos robos: ";
                
                for(int id : conflicting_robots) ss << id << " ";
                
                // Em vermelho para destacar
    ...
    std::vector<CollisionZone> detect_and_cluster_collisions() 
    {
        ...
        // Fase 2: Clustering Espacial (BFS)
        for(const auto& start_key : collision_candidates) 
        {
            if(visited_keys.count(start_key)) 
            {
                continue;
            }
            ...
            while(!queue.empty()) 
            {
                ...
                for(int i=0; i<4; i++) 
                {
                    ...
                    {
                        // Comparação otimizada via vetor
                        if(signature_cache[neighbor] == current_sig) 
                        {
                            visited_keys.insert(neighbor); 
        ...
        // Fase 3: Processamento Temporal (Paralelizado)
        ...
        for(size_t z_idx = 0; z_idx < zones.size(); z_idx++) 
        {
            ...
            for(const auto& pt : zone.points) 
            {
                ...
                if (it != global_occupancy_map.end()) 
                {
                    ...
                    {
                        if(zone.involved_robots.count(v.robot_id)) 
                        {
                            raw_times[v.robot_id].push_back(v.timestamp);
                        }
                    }
                }
            }

            // Criação de intervalos contínuos
        ...
        if (!collision_candidates.empty()) 
        {
            RCLCPP_INFO(this->get_logger(), "Collision Detection (Optimized): %.6f s | Zones Found: %zu", 
                duration.count(), zones.size());
        }
    ...
    std::pair<std::vector<ResolutionLog>, std::map<int, float>> analyze_and_resolve_conflicts(std::vector<CollisionZone>& zones)
    {
        ...
        {
            for(int rid : zone.involved_robots) 
            {
                all_robot_ids.insert(rid);
            }
        }
        ...
        for(int current_robot : all_robot_ids) 
        {
            ...
            while(factor_changed) 
            {
                ...
                for(const auto& zone : zones) 
                {
                    if(zone.robot_intervals.find(current_robot) == zone.robot_intervals.end()) 
                    {
                        continue;
                    }
    ...
    {
        if (original_paths_.find(robot_id) == original_paths_.end()) 
        {
            return {0.0f, {0,0}};
        }
        ...
        if (zone.points_set.count(start_k)) 
        {
            return {0.0f, path[0]}; 
        }
        ...
        for (size_t i = 0; i < path.size() - 1; ++i) 
        {
            ...
            while (t_dist < dist) 
            {
                ...
                if (zone.points_set.count(k)) 
                {
                    return {total_dist_traveled + t_dist, {cur_x, cur_y}};
                }
                t_dist += config_.resolution;
            }
    ...
    // =============================================================================
    // SEÇÃO DE VISUALIZAÇÃO E RELATÓRIOS
    // =============================================================================
    ...
    void generate_visualization_trajectories(const std::vector<ResolutionLog>& logs) 
    {
        ...
        for(const auto& log : logs) 
        {
            ...
            {
                if (log.required_speed < robot_speeds[log.r_victim])
                {
        ...
        for(const auto& [rid, path] : original_paths_) 
        {
            ...
            if(current_t > max_sim_time_) 
            {
                max_sim_time_ = current_t;
            }
        }
        max_sim_time_ += 2.0f; 
        is_simulating_ = true;
    ...
    void animation_loop() 
    {
        if(!is_simulating_ || animated_trajectories_.empty()) 
        {
            return;
        }
        
        visualization_msgs::msg::MarkerArray markers;
        visualization_msgs::msg::Marker del_mk; 
        del_mk.action = visualization_msgs::msg::Marker::DELETEALL; 
        markers.markers.push_back(del_mk);
        ...
        for(const auto& [rid, traj] : animated_trajectories_) 
        {
            ...
            robot_mk.header.frame_id = "world"; 
            robot_mk.header.stamp = this->now();
            robot_mk.ns = "simulated_robots"; 
            robot_mk.id = rid;
            robot_mk.type = visualization_msgs::msg::Marker::CYLINDER; 
            robot_mk.action = visualization_msgs::msg::Marker::ADD;
            robot_mk.pose.position.x = x; 
            robot_mk.pose.position.y = y; 
            robot_mk.pose.position.z = 0.2;
            robot_mk.scale.x = 0.4; 
            robot_mk.scale.y = 0.4; 
            robot_mk.scale.z = 0.4;
