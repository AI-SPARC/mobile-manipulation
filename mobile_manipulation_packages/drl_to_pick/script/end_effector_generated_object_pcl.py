...
# Renderiza a cada 500 steps de física (aprox 1.4s) para máxima velocidade
RENDER_SKIP = 500 
DENSITY_SAMPLES = 25000 
FORCE_THRESHOLD = 0.01  

# ==========================================
# 1. SISTEMA DE NÃO-REPETIÇÃO
# ==========================================
ALL_SHAPES_LIST = [
    ...
    # --- CARACTERES INDUSTRIAIS (A-Z, 0-9) ---
...
def apply_mesh_physx(prim, color=None):
    # Configuração de Colisão Milimétrica para o Objeto
    ...
    # 0.01% de erro = Colisão praticamente idêntica ao visual
...
def generate_industrial_object(stage):
    ...
    # Massa aumentada para 200g (0.2kg) para estabilidade
    ...
    # Lógica de Geração (Alta Resolução: 32 ou 64 lados para curvas)
...
# [OTIMIZAÇÃO] Tunando a garra para colisão perfeita
    ...
    for prim in Usd.PrimRange(stage.GetPrimAtPath(gripper_path)):
        if prim.IsA(UsdGeom.Mesh): 
            ...
            if not prim.HasAPI(PhysxSchema.PhysxConvexDecompositionCollisionAPI):
                PhysxSchema.PhysxConvexDecompositionCollisionAPI.Apply(prim)
            decomp_api = PhysxSchema.PhysxConvexDecompositionCollisionAPI(prim)
            # Parametros para colisão "Pixel Perfect"
            decomp_api.CreateErrorPercentageAttr(0.01) 
            decomp_api.CreateMaxConvexHullsAttr(64)
            decomp_api.CreateHullVertexLimitAttr(64)
            decomp_api.CreateShrinkWrapAttr(False)
            ...
            # Contact Offset 1mm para precisão máxima de toque
...
async def run_single_iteration(iteration_index, parent_output_dir):
    ...
    # [360Hz] Alta precisão temporal (Física roda 6x para cada frame 60fps)
    world.set_simulation_dt(physics_dt=1.0/540.0, rendering_dt=1.0/5.0)
    ...
    for i, pose in enumerate(poses):
        ...
        for phase in phases:
            ...
            # Multiplicamos a duração (segundos) pela frequência (360)
            target_duration = getattr(phase, "duration", 1.0) 
            steps = int(target_duration * 540) 
            
            apply_joint_targets(stage, getattr(phase, "joint_drive_targets", {}))
            
            for s in range(steps):
                # Roda a física pura (rápido)
                world.step(render=False) 
                
                # [SPEED HACK] Renderiza raramente para não travar a GPU
                if s % RENDER_SKIP == 0: 
                ...
                for sensor in sensor_map.values():
                    ...
                    f_sum += current_f
                    # Detecção Híbrida: Força ou Flag
        ...
        if stage.GetPrimAtPath(ee_prim_path):
            ...
            print(f"!!! AVISO: 'end_effector' não encontrado em {grasping_manager.gripper_path}. Usando pose root.")
        ...
        if max_f < FORCE_THRESHOLD and not success:
            status_msg = f"FAIL (Sem contato/Força Baixa: {max_f:.3f})"
        elif success:
