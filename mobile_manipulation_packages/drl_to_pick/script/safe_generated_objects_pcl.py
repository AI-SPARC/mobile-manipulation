...
OUTPUT_DIR_ROOT = os.path.join(os.getcwd(), "toma") 

# --- CONTROLE GERAL ---
TOTAL_ITERATIONS = 100 
# !!! LIMITE RÍGIDO DE 7.5cm (0.075m) !!!
GLOBAL_MAX_SIZE = 0.075  
THICKNESS_MIN = 0.005
THICKNESS_MAX = 0.035
...
# Sensor muito sensível para detectar qualquer toque na malha
FORCE_THRESHOLD = 0.01  

# ==========================================
# 1. SISTEMA DE NÃO-REPETIÇÃO (DECK MASSIVO)
# ==========================================
ALL_SHAPES_LIST = [
    # --- PRIMITIVOS BÁSICOS (10) ---
    ...
    # --- CARACTERES INDUSTRIAIS (A-Z, 0-9) - ÓTIMOS PARA GRASPING (36) ---
...
def get_next_unique_shape():
    ...
    if len(SHAPE_BUFFER) == 0:
        print(f">>> [SISTEMA] Recarregando Baralho ({len(ALL_SHAPES_LIST)} objetos)...")
        SHAPE_BUFFER = list(ALL_SHAPES_LIST)
        random.shuffle(SHAPE_BUFFER)
    return SHAPE_BUFFER.pop()

# ==========================================
# 2. GERADORES DE GEOMETRIA (MESHES UNIDAS)
# ==========================================
def apply_mesh_physx(prim, color=None):
    # [IMPORTANTE] convexDecomposition para colisão precisa em concavidades
...
def generate_industrial_object(stage):
    ...
    # Evita que o objeto entre em sleep e pare de gerar contatos
    ...
    colors = [
        Gf.Vec3f(0.8, 0.4, 0.0), Gf.Vec3f(0.9, 0.8, 0.0), 
        Gf.Vec3f(0.3, 0.3, 0.35), Gf.Vec3f(0.7, 0.7, 0.75),
        Gf.Vec3f(0.2, 0.25, 0.3), Gf.Vec3f(0.6, 0.1, 0.1)
    ]
    base_color = random.choice(colors)

    # ==========================================================
    # LÓGICA DE GERAÇÃO (BOUNDING BOX AWARE)
    # ==========================================================
    ...
    if shape_type == "SIMPLE_CUBE":
        ...
        create_prism_mesh(stage, obj_path, "c", L/2, L, 32, (0,0,0), (90,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "base", L/2, L*0.5, 32, (0,0, -L*0.25), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "top", L*0.25, L*0.5, 32, (0,0, L*0.25), (90,0,0), base_color)
    ...
    # --- TUBULAÇÃO ---
        ...
        r = L * 0.15
        create_prism_mesh(stage, obj_path, "m", r, L, 12, (0,0,0), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "b", r, L*0.5, 12, (0,0, L*0.25), (0,0,0), base_color)
    elif shape_type == "ELBOW_PIPE":
        l_len = L * 0.7 
        r = L * 0.15
        create_prism_mesh(stage, obj_path, "l1", r, l_len, 12, (l_len/2 - r, 0, 0), (0, 90, 0), base_color)
        create_prism_mesh(stage, obj_path, "l2", r, l_len, 12, (0, 0, l_len/2 - r), (0, 0, 0), base_color)
        create_box_mesh(stage, obj_path, "j", (r*2, r*2, r*2), (0,0,0), (0,0,0), base_color)
    elif shape_type == "CROSS_PIPE_4WAY":
        r = L * 0.15
        create_prism_mesh(stage, obj_path, "p1", r, L, 12, (0,0,0), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "p2", r, L, 12, (0,0,0), (0,90,0), base_color)
    elif shape_type == "FLANGE_ADAPTER":
        tr, fr = L * 0.15, L * 0.3
        tube_len = L - t
        create_prism_mesh(stage, obj_path, "t", tr, tube_len, 12, (0,0, t/2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "f", fr, t, 16, (0,0, -tube_len/2), (0,0,0), base_color)
    elif shape_type == "COUPLING_SLEEVE":
        create_prism_mesh(stage, obj_path, "o", L*0.25, L*0.8, 12, (0,0,0), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "r", L*0.3, L*0.2, 12, (0,0,0), (90,0,0), base_color)
    elif shape_type == "BUSHING_FLANGED":
        create_prism_mesh(stage, obj_path, "b", L*0.15, L*0.8, 12, (0,0, t/2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "h", L*0.25, t, 16, (0,0, -L*0.4 + t/2), (0,0,0), base_color)
    elif shape_type == "MANIFOLD_3PORT":
        r = L*0.12
        create_prism_mesh(stage, obj_path, "main", r, L, 12, (0,0,0), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "p1", r*0.8, L*0.3, 12, (-L*0.3, 0, L*0.2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "p2", r*0.8, L*0.3, 12, (0, 0, L*0.2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "p3", r*0.8, L*0.3, 12, (L*0.3, 0, L*0.2), (0,0,0), base_color)
    elif shape_type == "REDUCER_COUPLING":
        create_prism_mesh(stage, obj_path, "big", L*0.3, L*0.5, 12, (0,0, -L*0.25), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "sml", L*0.15, L*0.5, 12, (0,0, L*0.25), (0,0,0), base_color)
    elif shape_type == "END_CAP_HEX":
        create_prism_mesh(stage, obj_path, "hex", L*0.3, t*2, 6, (0,0, -L*0.2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "cap", L*0.25, L*0.4, 16, (0,0, t), (0,0,0), base_color)
    elif shape_type == "Y_PIPE_DIVIDER":
        create_prism_mesh(stage, obj_path, "m", L*0.15, L*0.4, 12, (0,0, -L*0.1), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "b1", L*0.12, L*0.3, 12, (L*0.1, 0, L*0.2), (0,30,0), base_color)
        create_prism_mesh(stage, obj_path, "b2", L*0.12, L*0.3, 12, (-L*0.1, 0, L*0.2), (0,-30,0), base_color)
    elif shape_type == "U_RETURN_BEND":
        create_prism_mesh(stage, obj_path, "l1", L*0.15, L*0.7, 12, (L*0.2, 0, 0), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "l2", L*0.15, L*0.7, 12, (-L*0.2, 0, 0), (90,0,0), base_color)
        create_box_mesh(stage, obj_path, "join", (L*0.7, L*0.3, L*0.3), (0, 0, L*0.35), (0,0,0), base_color)
    elif shape_type == "NIPPLE_PIPE":
        create_prism_mesh(stage, obj_path, "body", L*0.15, L*0.6, 12, (0,0,0), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "t1", L*0.16, L*0.2, 12, (0,0, L*0.3), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "t2", L*0.16, L*0.2, 12, (0,0, -L*0.3), (90,0,0), base_color)
    ...
    # --- FIXADORES ---
        ...
        create_prism_mesh(stage, obj_path, "w", L*0.4, t, 16, (0,0,0), (0,0,0), base_color)
    elif shape_type == "SPLIT_WASHER":
        create_prism_mesh(stage, obj_path, "w", L*0.4, t, 16, (0,0,0), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "cut", (L, t*2, t*2), (0, L*0.4, 0), (0,0,0), base_color) # Cutout
    elif shape_type == "BOLT_HEX_SHORT":
        create_prism_mesh(stage, obj_path, "h", L*0.3, L*0.2, 6, (0,0, L*0.2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s", L*0.15, L*0.4, 12, (0,0, -L*0.1), (0,0,0), base_color)
    elif shape_type == "BOLT_HEX_LONG":
        create_prism_mesh(stage, obj_path, "h", L*0.3, L*0.15, 6, (0,0, L*0.4), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s", L*0.15, L*0.8, 12, (0,0, -L*0.07), (0,0,0), base_color)
    elif shape_type == "BOLT_ALLEN":
        create_prism_mesh(stage, obj_path, "h", L*0.3, L*0.2, 12, (0,0, L*0.3), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s", L*0.15, L*0.6, 12, (0,0, -L*0.1), (0,0,0), base_color)
    elif shape_type == "SHAFT_COLLAR":
        create_prism_mesh(stage, obj_path, "c", L*0.3, L*0.15, 16, (0,0,0), (90,0,0), base_color)
        create_box_mesh(stage, obj_path, "s", (t, t, L*0.35), (0, L*0.25, 0), (0,0,0), base_color)
    elif shape_type == "WING_NUT":
        create_prism_mesh(stage, obj_path, "c", L*0.15, L*0.2, 12, (0,0,0), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "s", L*0.1, L*0.6, 12, (0,0, -L*0.2), (0,0,0), base_color)
        r = L*0.2
        create_prism_mesh(stage, obj_path, "t1", L*0.08, r*2, 8, (0, r, L*0.2), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "t2", L*0.08, r*2, 8, (0, -r, L*0.2), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "t3", L*0.08, r*2, 8, (r, 0, L*0.2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "t4", L*0.08, r*2, 8, (-r, 0, L*0.2), (0,0,0), base_color)
    elif shape_type == "TURNBUCKLE_BODY":
        create_box_mesh(stage, obj_path, "b", (L*0.4, t*2, L*0.6), (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "e1", L*0.1, L*0.2, 12, (0,0, L*0.4), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "e2", L*0.1, L*0.2, 12, (0,0, -L*0.4), (0,0,0), base_color)
    elif shape_type == "COTTER_PIN_MOCK":
        create_prism_mesh(stage, obj_path, "head", L*0.15, t, 12, (0,0, L*0.4), (90,0,0), base_color)
    ...
    elif shape_type == "CARRIAGE_BOLT":
        create_prism_mesh(stage, obj_path, "h", L*0.4, L*0.1, 16, (0,0, L*0.45), (0,0,0), base_color) 
        create_box_mesh(stage, obj_path, "sq", (L*0.2, L*0.2, L*0.1), (0,0, L*0.35), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s", L*0.1, L*0.7, 12, (0,0, -L*0.05), (0,0,0), base_color)
    elif shape_type == "CASTLE_NUT":
        create_prism_mesh(stage, obj_path, "base", L*0.4, L*0.2, 6, (0,0, -L*0.1), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "top", L*0.3, L*0.15, 12, (0,0, L*0.1), (0,0,0), base_color)
    elif shape_type == "ACORN_NUT":
        create_prism_mesh(stage, obj_path, "base", L*0.4, L*0.25, 6, (0,0, -L*0.1), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "dome", L*0.35, L*0.2, 12, (0,0, L*0.15), (0,0,0), base_color)
    elif shape_type == "U_BOLT_PLATE":
        create_box_mesh(stage, obj_path, "plate", (L*0.6, L*0.2, t), (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "l1", t*2, L*0.5, 8, (L*0.2, 0, L*0.25), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "l2", t*2, L*0.5, 8, (-L*0.2, 0, L*0.25), (0,0,0), base_color)
    elif shape_type == "SET_SCREW":
        create_prism_mesh(stage, obj_path, "body", L*0.15, L*0.3, 12, (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "hex", L*0.08, L*0.32, 6, (0,0,0), (0,0,0), base_color) # hole simulation
    elif shape_type == "THUMB_SCREW":
        create_prism_mesh(stage, obj_path, "head", L*0.4, t*2, 12, (0,0, L*0.4), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s", L*0.1, L*0.8, 12, (0,0, 0), (0,0,0), base_color)
    elif shape_type == "RIVET_MOCK":
        create_prism_mesh(stage, obj_path, "head", L*0.25, t, 16, (0,0, L*0.3), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "body", L*0.1, L*0.6, 12, (0,0, 0), (0,0,0), base_color)
    elif shape_type == "CLEVIS_PIN":
        create_prism_mesh(stage, obj_path, "head", L*0.25, t, 16, (0,0, L*0.45), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "body", L*0.12, L*0.9, 12, (0,0, 0), (0,0,0), base_color)

    # --- MECÂNICA ---
    elif "SPUR_GEAR" in shape_type:
        teeth_map = {"6T":6, "8T":8, "12T":12, "24T":12} # 24 simplified
        n = teeth_map.get(shape_type.split("_")[-1], 8)
        create_prism_mesh(stage, obj_path, "c", L*0.4, t*2, 16, (0,0,0), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "b", L*0.25, t, 16, (0, -L*0.1, 0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "tp", L*0.15, t, 12, (0, L*0.2, 0), (0,0,0), base_color)
    elif shape_type == "VALVE_HANDWHEEL":
        rw = L * 0.45 
        create_prism_mesh(stage, obj_path, "c1", t, rw*2, 8, (rw, 0, 0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "c2", t, rw*2, 8, (-rw, 0, 0), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "h", L*0.25, L*0.4, 12, (0,0, L*0.1), (90,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "pn", t*1.5, L*0.5, 8, (0,0,0), (0,0,0), base_color)
    elif shape_type == "DOWEL_PIN":
        create_prism_mesh(stage, obj_path, "p", L*0.05, L, 8, (0,0,0), (90,0,0), base_color)
    elif shape_type == "SPROCKET_THIN":
        create_prism_mesh(stage, obj_path, "c", L*0.45, t, 16, (0,0,0), (0,0,0), base_color)
        for i in range(4):
            create_box_mesh(stage, obj_path, f"t{i}", (L, L*0.1, t), (0,0,0), (0,0, i*45), base_color)
    elif shape_type == "CONNECTING_ROD":
        create_prism_mesh(stage, obj_path, "end1", L*0.25, t*2, 12, (L*0.35, 0, 0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "end2", L*0.15, t*2, 12, (-L*0.35, 0, 0), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "bar", (L*0.7, L*0.1, t), (0,0,0), (0,0,0), base_color)
    elif shape_type == "PISTON_HEAD":
        create_prism_mesh(stage, obj_path, "head", L*0.3, L*0.4, 16, (0,0, L*0.2), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "pin", (L*0.2, L*0.2, L*0.2), (0,0, -L*0.1), (0,0,0), base_color)
    elif shape_type == "BEARING_LINEAR_BLOCK":
        create_box_mesh(stage, obj_path, "b", (L*0.6, L*0.8, L*0.5), (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "rail", L*0.15, L*0.82, 12, (0,0,0), (90,0,0), base_color) 
    elif shape_type == "PULLEY_V_BELT":
        create_prism_mesh(stage, obj_path, "s1", L*0.4, t, 16, (0,0, t), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s2", L*0.4, t, 16, (0,0, -t), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "in", L*0.3, t, 16, (0,0, 0), (0,0,0), base_color)
    elif shape_type == "PULLEY_TIMING":
        create_prism_mesh(stage, obj_path, "main", L*0.4, L*0.3, 16, (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "f1", L*0.45, t, 16, (0,0, L*0.15), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "f2", L*0.45, t, 16, (0,0, -L*0.15), (0,0,0), base_color)
    elif shape_type == "UNIVERSAL_JOINT_CENTER":
        create_box_mesh(stage, obj_path, "c", (L*0.3, L*0.3, L*0.3), (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "p1", L*0.1, L, 8, (0,0,0), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "p2", L*0.1, L, 8, (0,0,0), (0,90,0), base_color)
    elif shape_type == "CRANKSHAFT_SEGMENT":
        create_prism_mesh(stage, obj_path, "m", L*0.15, L*0.2, 12, (0,0, -L*0.3), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "o", L*0.15, L*0.2, 12, (L*0.2, 0, L*0.3), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "web", (L*0.4, L*0.1, L*0.6), (L*0.1, 0, 0), (0,0,0), base_color)
    elif shape_type == "BEARING_BALL_MOCK":
        create_prism_mesh(stage, obj_path, "out", L*0.4, L*0.2, 16, (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "in", L*0.2, L*0.2, 16, (0,0,0), (0,0,0), base_color)
    elif shape_type == "WORM_GEAR_MOCK":
        create_prism_mesh(stage, obj_path, "s", L*0.15, L, 8, (0,0,0), (0,0,0), base_color)
        for i in range(5):
            z = -L*0.3 + i*L*0.15
            create_prism_mesh(stage, obj_path, f"th{i}", L*0.2, t, 12, (0,0, z), (15,0,0), base_color)
    ...
    # --- ELETRÔNICA ---
        ...
        create_prism_mesh(stage, obj_path, "c", L*0.15, L*0.5, 8, (0,0,0), (0,0,0), base_color)
        for i in range(4):
            create_box_mesh(stage, obj_path, f"f{i}", (L, t, L*0.5), (0,0,0), (0,0, i*45), base_color)
    elif shape_type == "FAN_BLADE_MOCK":
        create_prism_mesh(stage, obj_path, "hub", L*0.15, t*2, 8, (0,0,0), (0,0,0), base_color)
        for i in range(4):
            create_box_mesh(stage, obj_path, f"b{i}", (L*0.4, L*0.15, t), (L*0.25, 0, 0), (15,0, i*90), base_color)
    elif shape_type == "IMPELLER_MOCK":
        create_prism_mesh(stage, obj_path, "hub", L*0.15, t*4, 8, (0,0,0), (0,0,0), base_color)
        for i in range(6):
            create_box_mesh(stage, obj_path, f"b{i}", (L*0.3, t, L*0.2), (L*0.2, 0, 0), (0,0, i*60), base_color)
    elif shape_type == "BATTERY_PACK":
        create_box_mesh(stage, obj_path, "b", (L*0.8, L*0.5, L*0.3), (0,0,0), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "top", (L*0.7, L*0.4, L*0.05), (0,0, L*0.175), (0,0,0), base_color)
    elif shape_type == "RELAY_CUBE":
        create_box_mesh(stage, obj_path, "c", (L*0.6, L*0.6, L*0.5), (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "p1", L*0.05, L*0.2, 4, (L*0.2, 0, -L*0.3), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "p2", L*0.05, L*0.2, 4, (-L*0.2, 0, -L*0.3), (0,0,0), base_color)
    elif shape_type == "FUSE_CYLINDER":
        create_prism_mesh(stage, obj_path, "body", L*0.1, L*0.6, 8, (0,0,0), (0,90,0), base_color)
        create_prism_mesh(stage, obj_path, "c1", L*0.11, L*0.15, 12, (L*0.3, 0, 0), (0,90,0), base_color)
        create_prism_mesh(stage, obj_path, "c2", L*0.11, L*0.15, 12, (-L*0.3, 0, 0), (0,90,0), base_color)
    elif shape_type == "PCB_BOARD_POPULATED":
        create_box_mesh(stage, obj_path, "pcb", (L, L*0.6, t), (0,0, -t), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "chip", (L*0.3, L*0.3, t*2), (0,0, t), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "cap", L*0.1, L*0.2, 8, (L*0.3, 0, t), (0,0,0), base_color)
    elif shape_type == "DIP_CHIP_MOCK":
        create_box_mesh(stage, obj_path, "body", (L*0.8, L*0.3, t*2), (0,0,0), (0,0,0), base_color)
        for i in range(4):
            x = -L*0.3 + i*L*0.2
            create_box_mesh(stage, obj_path, f"l{i}", (t, L*0.4, t), (x, 0, -t*1.5), (0,0,0), base_color)
    elif shape_type == "TRANSFORMER_COIL":
        create_box_mesh(stage, obj_path, "core", (L*0.6, L*0.2, L*0.6), (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "coil", L*0.25, L*0.5, 12, (0,0,0), (0,0,0), base_color)
    elif shape_type == "TOGGLE_SWITCH_BODY":
        create_box_mesh(stage, obj_path, "body", (L*0.4, L*0.3, L*0.4), (0,0, -L*0.1), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "lever", L*0.05, L*0.4, 8, (0,0, L*0.2), (15,0,0), base_color)
    elif shape_type == "USB_CONNECTOR_MOCK":
        create_box_mesh(stage, obj_path, "met", (L*0.3, L*0.4, t*2), (0,0,0), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "ins", (L*0.3, L*0.2, t*2), (0, -L*0.3, 0), (0,0,0), base_color)
    elif shape_type == "SD_CARD_MOCK":
        create_box_mesh(stage, obj_path, "card", (L*0.6, L*0.8, t), (0,0,0), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "notch", (L*0.1, L*0.2, t), (L*0.25, L*0.3, 0), (0,0,0), base_color)

    # --- SUPORTES E FERRAMENTAS ---
    elif shape_type == "STAR_KNOB":
        create_prism_mesh(stage, obj_path, "c", L*0.2, L*0.2, 12, (0,0,0), (0,0,0), base_color)
        al = L*0.4
        for ang, n in zip([0,45,90,135], ["a1","a2","a3","a4"]):
            create_box_mesh(stage, obj_path, n, (al*2, t, t), (0,0,0), (0,0,ang), base_color)
    elif shape_type == "KEY_STOCK":
        create_box_mesh(stage, obj_path, "k", (L, t, t), (0,0,0), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "s", t, shaft_len, 12, (0,0,0), (0,90,0), base_color)
        create_prism_mesh(stage, obj_path, "w1", L*0.2, weight_w, 16, (-L/2 + weight_w/2, 0, 0), (0,90,0), base_color)
        create_prism_mesh(stage, obj_path, "w2", L*0.2, weight_w, 16, (L/2 - weight_w/2, 0, 0), (0,90,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "c", cr, ch, 12, (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "t", fr, fh, 16, (0,0, ch/2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "b", fr, fh, 16, (0,0, -ch/2), (0,0,0), base_color)
        ...
        bh, rh, w = L*0.6, L*0.4, L*0.6
        create_box_mesh(stage, obj_path, "b", (w, w, bh), (0,0, -rh/2), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "r", w/math.sqrt(2), w, 3, (0,0, bh/2), (90, 0, 0), base_color)
    elif shape_type == "HEX_WRENCH_L":
        hr, l1, l2 = L*0.1, L*0.8, L*0.4
        create_prism_mesh(stage, obj_path, "l1", hr, l1, 6, (0, l1/2, 0), (90, 0, 0), base_color)
        create_prism_mesh(stage, obj_path, "l2", hr, l2, 6, (l2/2, 0, 0), (0, 90, 0), base_color)
    elif shape_type == "STEPPED_SHAFT":
        ls = L/3
        create_prism_mesh(stage, obj_path, "s1", L*0.3, ls, 12, (0,0, -ls), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s2", L*0.2, ls, 12, (0,0, 0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s3", L*0.1, ls, 12, (0,0, ls), (0,0,0), base_color)
    ...
    elif shape_type == "BRIDGE_ARCH":
        ph, pw, sp = L*0.6, L*0.2, L*0.8
        create_box_mesh(stage, obj_path, "p1", (pw, pw, ph), (-sp/2, 0, -L*0.1), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "p2", (pw, pw, ph), (sp/2, 0, -L*0.1), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "t", (sp+pw, pw, t*2), (0, 0, ph/2), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "h", L*0.25, L*0.22, 16, (0,0,0), (90,0,0), base_color)
    elif shape_type == "MOTOR_MOUNT_PLATE":
        create_box_mesh(stage, obj_path, "p", (L*0.8, L*0.8, t), (0,0,0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "c", L*0.25, t*2, 16, (0,0,0), (0,0,0), base_color)
    ...
    elif shape_type == "MAGNET_HORSESHOE":
        create_prism_mesh(stage, obj_path, "arch", t*2, L*0.4, 12, (0,0, L*0.2), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "l1", t*2, L*0.5, 8, (-L*0.2, 0, -L*0.1), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "l2", t*2, L*0.5, 8, (L*0.2, 0, -L*0.1), (0,0,0), base_color)
    elif shape_type == "ANVIL_SHAPE":
        create_box_mesh(stage, obj_path, "b", (L*0.8, L*0.3, L*0.2), (0,0, -L*0.2), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "top", (L*0.4, L*0.25, L*0.2), (L*0.1, 0, 0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "horn", L*0.1, L*0.3, 4, (-L*0.25, 0, 0), (0,90,0), base_color)
    elif shape_type == "WHEEL_CASTER_MOCK":
        create_prism_mesh(stage, obj_path, "w", L*0.25, t*2, 16, (0,0, -L*0.15), (90,0,0), base_color)
        create_box_mesh(stage, obj_path, "m", (L*0.3, L*0.3, t), (0,0, L*0.2), (0,0,0), base_color)
        create_box_mesh(stage, obj_path, "ax", (t, t, L*0.3), (L*0.1, 0, 0), (0,0,0), base_color)
    elif shape_type == "HANDLE_PULL":
        create_prism_mesh(stage, obj_path, "h", t, L*0.8, 8, (0,0, L*0.1), (90,0,0), base_color)
        create_prism_mesh(stage, obj_path, "l1", t, L*0.2, 8, (L*0.3, 0, 0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "l2", t, L*0.2, 8, (-L*0.3, 0, 0), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "cyl", L*0.2, L*0.4, 12, (0,0,0), (0,0,0), base_color)
    ...
    # --- ALPHANUMERIC / STENCIL SHAPES (Simplified Blocky) ---
    elif shape_type.startswith("CHAR_"):
        char = shape_type.split("_")[1]
        # Basic logical construction for common block letters to serve as complex concave shapes
        # All fit in L x L box
        h, w = L, L*0.7
        th = t
        if char in "ABDOPQR": # Loop shapes (simulated with 4 bars)
        ...
        elif char in "STXYZ": # Crosses or Zs
            create_box_mesh(stage, obj_path, "c", (th, th, h), (0,0,0), (0,0,0), base_color)
        ...
        # Simple placeholder block for digits, using frame logic
...
async def run_single_iteration(iteration_index, parent_output_dir):
    ...
    for i, pose in enumerate(poses):
        ...
        for phase in phases:
            steps = getattr(phase, "simulation_steps", 60)
            apply_joint_targets(stage, getattr(phase, "joint_drive_targets", {}))
            
            for s in range(steps):
                world.step(render=False) 
                if s % RENDER_SKIP == 0: await omni.kit.app.get_app().next_update_async()

                f_sum = 0.0
                touch = 0
                for sensor in sensor_map.values():
                    reading = sensor.get_current_frame()
                    if reading and "force" in reading:
        ...
        if max_f < FORCE_THRESHOLD:
            success = False
            status_msg = f"FAIL (Força Baixa: {max_f:.1f})"
        elif not success:
            max_f = 0.0
            status_msg = "FAIL (Sem contato)"
        else:
            status_msg = f"SUCCESS (Força: {max_f:.1f})"

        print(f"Pose {i+1}/{len(poses)}: {status_msg}")

        p_lst = [float(x) for x in pose[0]]
        r_lst = [pose[1].GetReal()] + list(pose[1].GetImaginary())
