# ==========================================
# 1. BOOTSTRAP DO ISAAC SIM
from omni.isaac.kit import SimulationApp

# [CORREÇÃO FINAL]
# 1. headless: True -> Modo terminal real.
# 2. renderer: "RayTracedLighting" -> Obrigatório ser String para não dar erro de Python.
# 3. sync_loads: True -> Ajuda a evitar conflitos de carregamento.
config = {
    "headless": True,
...
# [CORREÇÃO DE CRASH]
# Estas linhas são o segredo para não dar Segmentation Fault no modo Headless.
# Elas desligam o sistema Fabric que tenta desenhar coisas que não existem no terminal.
import carb.settings
settings = carb.settings.get_settings()
settings.set_bool("/app/useFabricSceneDelegate", False)
settings.set_bool("/physics/fabricEnabled", False) 
settings.set_bool("/app/renderer/skipWhileMinimized", True)

# ==========================================
# 2. ATIVAÇÃO DE EXTENSÕES
# ==========================================
import isaacsim.core.utils.extensions as extensions

# Ativa extensões
extensions.enable_extension("omni.replicator.core")
extensions.enable_extension("isaacsim.replicator.grasping")

simulation_app.update()

# ==========================================
# 3. IMPORTS (SEM ASYNCIO)
# ==========================================
try:
    from isaacsim.replicator.grasping import GraspingManager
except ImportError:
    print("!!! ERRO CRÍTICO: Falha ao importar GraspingManager.")
...
OUTPUT_DIR_ROOT = os.path.join(os.getcwd(), "toma1") 

TOTAL_ITERATIONS = 500 
GLOBAL_MAX_SIZE = 0.07 
THICKNESS_MIN = 0.01
THICKNESS_MAX = 0.035
...
ALL_SHAPES_LIST = [
    ...
    "PYRAMID_TRUNCATED", "CONE_TRUNCATED", "L_SHAPE", "T_SHAPE", 
    "U_BRACKET", "I_BEAM", "C_CHANNEL", "Z_SHAPE", "E_SHAPE", 
    "H_BEAM", "CROSS_PROFILE", "ANGLE_BRACKET_V", "STRUT_CHANNEL", 
    "JOIST_HANGER", "CORNER_BRACE_3WAY", "RAIL_PROFILE", "SLOTTED_RAIL",
    "T_PIPE_JOINT", "ELBOW_PIPE", "CROSS_PIPE_4WAY", "FLANGE_ADAPTER", 
    "COUPLING_SLEEVE", "BUSHING_FLANGED", "MANIFOLD_3PORT", 
    "REDUCER_COUPLING", "END_CAP_HEX", "Y_PIPE_DIVIDER", "U_RETURN_BEND", 
    "NIPPLE_PIPE", "HEX_NUT", "SQUARE_NUT", "WING_NUT", "CASTLE_NUT", 
    "ACORN_NUT", "THICK_WASHER", "SPLIT_WASHER", "BOLT_HEX_SHORT", 
    "BOLT_HEX_LONG", "BOLT_ALLEN", "BOLT_CARRIAGE", "EYE_BOLT", 
    "U_BOLT_PLATE", "SET_SCREW", "THUMB_SCREW", "RIVET_MOCK", 
    "COTTER_PIN_MOCK", "DOWEL_PIN", "CLEVIS_PIN", "SHAFT_COLLAR",
    "SPUR_GEAR_6T", "SPUR_GEAR_8T", "SPUR_GEAR_12T", "SPUR_GEAR_24T",
    "RACK_GEAR", "WORM_GEAR_MOCK", "SPROCKET_THIN", "PULLEY_V_BELT", 
    "PULLEY_TIMING", "CAM_LOBE", "CRANKSHAFT_SEGMENT", 
    "UNIVERSAL_JOINT_CENTER", "CONNECTING_ROD", "PISTON_HEAD", 
    "BEARING_BALL_MOCK", "CLEVIS_MOUNT", "PILLOW_BLOCK", "HINGE_ASSEMBLY", 
    "BEARING_HOUSING", "MOTOR_MOUNT_PLATE", "STEP_BLOCK", "V_BLOCK", 
    "PARALLEL_BAR", "GUSSET_PLATE", "TRIANGLE_BRACKET", "SHELF_BRACKET", 
    "DIN_RAIL_SEGMENT", "HEAT_SINK_FINS", "HEAT_SINK_RADIAL", 
    "FAN_BLADE_MOCK", "IMPELLER_MOCK", "VALVE_HANDWHEEL", "STAR_KNOB",
    "HAMMER_HEAD", "WRENCH_OPEN_END", "WRENCH_BOX_END", "HEX_KEY_L", 
    "SOCKET_MOCK", "DRIVER_BIT_PHILLIPS", "DRIVER_BIT_FLAT", "PADLOCK_BODY", 
    "CARABINER_D", "CHAIN_LINK", "CHAR_A", "CHAR_B", "CHAR_C", "CHAR_D", 
    ...
    "CHAR_Z", "DIGIT_0", "DIGIT_1", "DIGIT_2", "DIGIT_3", "DIGIT_4", 
...
def generate_industrial_object(stage):
    ...
    # Remove objeto anterior se existir para evitar sobreposição
    ...
    print(f"  -> Gerando: {shape_type} | Box: {L*100:.1f}cm | t: {t*1000:.1f}mm | Massa: 200g")
    ...
    if shape_type == "SIMPLE_CUBE":
        ...
        create_prism_mesh(stage, obj_path, "e1", L*0.1, L*0.2, 32, (0,0, L*0.4), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "e2", L*0.1, L*0.2, 32, (0,0, -L*0.4), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "rail", L*0.15, L*0.82, 32, (0,0,0), (90,0,0), base_color) 
        ...
        create_prism_mesh(stage, obj_path, "body", L*0.1, L*0.6, 16, (0,0,0), (0,90,0), base_color)
        create_prism_mesh(stage, obj_path, "c1", L*0.11, L*0.15, 32, (L*0.3, 0, 0), (0,90,0), base_color)
        create_prism_mesh(stage, obj_path, "c2", L*0.11, L*0.15, 32, (-L*0.3, 0, 0), (0,90,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "cap", L*0.1, L*0.2, 16, (L*0.3, 0, t), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "coil", L*0.25, L*0.5, 32, (0,0,0), (0,0,0), base_color)
    elif shape_type == "TOGGLE_SWITCH_BODY":
        create_box_mesh(stage, obj_path, "body", (L*0.4, L*0.3, L*0.4), (0,0, -L*0.1), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "lever", L*0.05, L*0.4, 16, (0,0, L*0.2), (15,0,0), base_color)
        ...
        ls = L/3
        create_prism_mesh(stage, obj_path, "s1", L*0.3, ls, 32, (0,0, -ls), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s2", L*0.2, ls, 32, (0,0, 0), (0,0,0), base_color)
        create_prism_mesh(stage, obj_path, "s3", L*0.1, ls, 32, (0,0, ls), (0,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "arch", t*2, L*0.4, 16, (0,0, L*0.2), (90,0,0), base_color)
        ...
        create_prism_mesh(stage, obj_path, "w", L*0.25, t*2, 32, (0,0, -L*0.15), (90,0,0), base_color)
        ...
        # Fallback genérico se o shape não tiver if específico
        create_box_mesh(stage, obj_path, "fallback_cube", (L, L, L), (0,0,0), (0,0,0), base_color)
...
def improve_gripper_physics(stage, gripper_path):
    print(f"--- OTIMIZANDO FÍSICA DA GARRA: {gripper_path} ---")
...
# ==========================================
# 9. EXECUÇÃO SÍNCRONA (CORRIGIDA)
# ==========================================

# [CORREÇÃO] Função síncrona. Força o avanço da simulação frame a frame.
def step_simulation(n):
    for _ in range(n):
        simulation_app.update()

def run_single_iteration(iteration_index, parent_output_dir):
    ...
    stage = omni.usd.get_context().get_stage()
    if not stage:
        print("ERRO CRÍTICO: Stage perdido ou não carregado!")
        return

    print(">> Gerando Objeto...")
    generated_obj_path = generate_industrial_object(stage)
    
    # [CORREÇÃO] Steps síncronos
    step_simulation(10)
    ...
    world = World() 
    # [CORREÇÃO] Inicialização síncrona (removido _async)
    world.initialize_simulation_context()
    world.set_simulation_dt(physics_dt=1.0/360.0, rendering_dt=1.0/60.0)
    
    # [CORREÇÃO] Reset síncrono
    world.reset()
    step_simulation(5)
    ...
    print(f">> Iniciando Teste de {len(poses)} Poses...")
    ...
    for i, pose in enumerate(poses):
        # [CORREÇÃO] Update manual para manter o app vivo
        if i % 5 == 0: simulation_app.update()
        ...
        for phase in phases:
            ...
            for s in range(steps):
                ...
                # [CORREÇÃO] Update síncrono e direto
                if s % RENDER_SKIP == 0: 
                    simulation_app.update()
...
def run_batch_process():
    ...
    # Aquece o simulador (Síncrono)
    step_simulation(100)

    print(">> Abrindo Stage Principal (UMA VEZ)...")
    if not os.path.exists(STAGE_PATH):
        print(f"ERRO: Stage não encontrado em {STAGE_PATH}")
        return

    # Abre o stage SÍNCRONO
    ...
    for i in range(TOTAL_ITERATIONS):
        ...
        # Chamada direta, sem await
        run_single_iteration(i, OUTPUT_DIR_ROOT)
        
    print("BATCH CONCLUÍDO!")

# [CORREÇÃO] Execução direta, sem loop de eventos asyncio
