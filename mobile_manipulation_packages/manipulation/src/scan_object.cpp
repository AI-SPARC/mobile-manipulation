   ...
   robot_pos_received_(false)
{
    ...
    sub_detections_ = this->create_subscription<vision_msgs::msg::Detection3DArray>(
        "/bbox_3d_with_labels", 10, std::bind(&ScanObject::detectionCallback, this, std::placeholders::_1));

    sub_semantic_pcl_ = this->create_subscription<mobile_manipulation_interfaces::msg::SemanticPcl>(
        "/semantic_pcl_array", 10, std::bind(&ScanObject::semanticPclCallback, this, std::placeholders::_1));

    sub_odometry_ = this->create_subscription<nav_msgs::msg::Odometry>(
        odom_topic_, 10, std::bind(&ScanObject::odometryCallback, this, std::placeholders::_1));

    marker_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/visualization_marker_array", 10);
    animation_timer_ = this->create_wall_timer(200ms, std::bind(&ScanObject::animationTimerCallback, this));

    RCLCPP_INFO(this->get_logger(), "ScanObject: Modo Greedy Online (2 Passadas) Iniciado.");
}


std::vector<TargetVoxel> ScanObject::generateTargetVoxels(const vision_msgs::msg::Detection3D& detection) {
...
bool ScanObject::isVoxelVisible(const geometry_msgs::msg::Pose& pose, const TargetVoxel& voxel) {
...
std::vector<geometry_msgs::msg::Pose> ScanObject::filterPosesByCoverage(
