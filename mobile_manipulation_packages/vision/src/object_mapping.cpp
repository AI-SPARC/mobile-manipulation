   ...
   is_robot_stopped_(false)
{
    ...
    pub_semantic_environment_ = this->create_publisher<mobile_manipulation_interfaces::msg::SemanticPcl>(
        "/mapped_environment_semantic", 10);

    pub_environment_cloud_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "/mapped_environment_cloud", 10);

    RCLCPP_INFO(this->get_logger(), "ObjectMapping iniciado. Voxel: %.3fm | Raio Ambiente: %.2fm", 
        voxel_leaf_size_, surrounding_distance_threshold_);
}
...
void ObjectMapping::semanticPclCallback(const mobile_manipulation_interfaces::msg::SemanticPcl::SharedPtr msg) 
{
    ...
    publishSemanticEnvironment(msg, current_target);
}


void ObjectMapping::publishSemanticEnvironment(
    ...
    const std::string& target_label)
{
    ...
    const MappingObjectData& target_data = object_map_.at(target_label);
    ...
    mobile_manipulation_interfaces::msg::SemanticPcl output_msg;
    ...
    target_cloud_msg.header.stamp = output_msg.header.stamp;

    output_msg.labels.push_back(target_label);
    output_msg.poses.push_back(target_data.pose);
    output_msg.clouds.push_back(target_cloud_msg);
    ...
    for (size_t i = 0; i < input_msg->labels.size(); ++i) 
    {
        ...
        if ((dx*dx + dy*dy + dz*dz) > (surrounding_distance_threshold_ * surrounding_distance_threshold_)) {
            continue; 
        }
        ...
        if (!filtered_env_cloud->empty())
        {
            ...
            output_msg.labels.push_back(current_env_label);
            output_msg.poses.push_back(input_msg->poses[i]);
            output_msg.clouds.push_back(filtered_cloud_msg);
        }
    }


    if (!output_msg.labels.empty()) 
    {
        pub_semantic_environment_->publish(output_msg);
    }

    
    if (!combined_visual_cloud.empty()) 
    {
        sensor_msgs::msg::PointCloud2 visual_msg;
        pcl::toROSMsg(combined_visual_cloud, visual_msg);
        visual_msg.header.frame_id = "world";
        visual_msg.header.stamp = output_msg.header.stamp;
...
void ObjectMapping::publishToPlanningScene()
{
    ...
    for (const auto& [label, data] : object_map_) {
        if (!data.cloud->empty()) { has_points = true; break; }
    }
