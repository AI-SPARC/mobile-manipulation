...
int32_t CombinedSemanticPCL::getPixelId(const sensor_msgs::msg::Image::ConstSharedPtr & img, size_t index)
{
    if (index >= (img->height * img->width)) return 0;

    if (img->encoding == "mono8") {
        return static_cast<int32_t>(img->data[index]);
    } 
    else if (img->encoding == "16UC1" || img->encoding == "mono16") {
        const uint16_t* ptr = reinterpret_cast<const uint16_t*>(img->data.data());
        return static_cast<int32_t>(ptr[index]);
    }
    else {
        const int32_t* ptr = reinterpret_cast<const int32_t*>(img->data.data());
        return ptr[index];
    }
}
...
{
    if (obj_id == 0) return std::make_tuple(60, 60, 60); 
    ...
    const std_msgs::msg::Header & header)
{
    ...
    {
        int32_t current_pixel_id = semantic_ids[i];
        
        if (id_to_label_.count(current_pixel_id)) 
        {
            std::string label_str = id_to_label_.at(current_pixel_id);
            if (!label_str.empty()) 
            {
