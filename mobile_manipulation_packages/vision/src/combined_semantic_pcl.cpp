      ...
      frame_count_(0)
{
    ...
    this->declare_parameter<std::string>("topic_detections", "/bbox_3d_with_labels");
    this->declare_parameter<std::string>("topic_object_data", "/object_data");
    ...
    std::string topic_detections = this->get_parameter("topic_detections").as_string();
    std::string topic_object_data = this->get_parameter("topic_object_data").as_string();
    ...
    RCLCPP_INFO(this->get_logger(), "Ouvindo Detections em: %s", topic_detections.c_str());
    RCLCPP_INFO(this->get_logger(), "Publicando ObjectData em: %s", topic_object_data.c_str());
    ...
    detections_sub_ = this->create_subscription<vision_msgs::msg::Detection3DArray>(
        topic_detections,
        10,
        std::bind(&CombinedSemanticPCL::detectionsCallback, this, std::placeholders::_1)
    );
    ...
    pub_object_data_ = this->create_publisher<mobile_manipulation_interfaces::msg::ObjectData>(topic_object_data, 10);
}
...
void CombinedSemanticPCL::detectionsCallback(const vision_msgs::msg::Detection3DArray::SharedPtr msg)
{
...
std::tuple<uint8_t, uint8_t, uint8_t> CombinedSemanticPCL::getColorForId(int32_t obj_id)
{
    ...
    auto color = std::make_tuple((uint8_t)dis(gen), (uint8_t)dis(gen), (uint8_t)dis(gen));
    ...
    const sensor_msgs::msg::PointCloud2::ConstSharedPtr & pcl_msg)
{
    ...
    tf2::fromMsg(t_stamped.transform, cam_to_world);
    ...
    std::map<int32_t, std::vector<std::array<float, 3>>> object_points_map;
    ...
    for (size_t i = 0; i < num_points; i += downsample_step_)
    {
        ...
        tf2::Vector3 pt_world = cam_to_world * pt_cam;
        std::array<float, 3> p_final = {(float)pt_world.x(), (float)pt_world.y(), (float)pt_world.z()};

        
        int32_t sem_id = 0;
        if (num_points == total_pixels) sem_id = seg_data[i];
        else {
        ...
        world_points.push_back(p_final);
        valid_ids.push_back(sem_id);

        
        if (sem_id > 0) {
            object_points_map[sem_id].push_back(p_final);
        }
    }
    ...
    vision_msgs::msg::Detection3DArray::SharedPtr local_detections;
    {
        std::lock_guard<std::mutex> lock(detections_mutex_);
        if (last_detections_) local_detections = last_detections_;
    }

    if (local_detections && !local_detections->detections.empty())
    {
        for (const auto& det : local_detections->detections)
        {
            if (det.results.empty()) continue; 

            std::string label_str = det.results[0].hypothesis.class_id;
            
            
            int32_t target_id = -1;
            
            
            if (label_to_id_.count(label_str)) {
                target_id = label_to_id_[label_str];
            } 
            else {
                for (auto const& [name, id] : label_to_id_) {
                    if (name.find(label_str) != std::string::npos || label_str.find(name) != std::string::npos) {
                        target_id = id;
                        break;
                    }
                }
            ...
            if (target_id != -1 && object_points_map.count(target_id) > 0)
            {
                const auto& specific_cloud = object_points_map[target_id];

               
                if (specific_cloud.size() > 50)
                {
                    mobile_manipulation_interfaces::msg::ObjectData obj_msg;
                    obj_msg.header = output_header; 
                    obj_msg.label = label_str;
                    
                   
                    
                    obj_msg.pose = det.bbox.center;
                    obj_msg.size = det.bbox.size;
                    
                   
                    obj_msg.object_cloud = createPCLMsg(specific_cloud, output_header);
                    
                    pub_object_data_->publish(obj_msg);
                }
            }
        }
    }
    ...
    if (frame_count_ % 30 == 0) {
        RCLCPP_INFO(this->get_logger(), "Publicado. Pontos totais: %zu", world_points.size());
    }
}
...
int main(int argc, char ** argv)
{
    ...
    auto node = std::make_shared<semantic_pcl::CombinedSemanticPCL>(rclcpp::NodeOptions());
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}